...
public class LoginActivity extends Activity
{
		...
		public void handleMessage(Message msg)
		{
			...
			case LOGIN_SUCCESS:
			{
				...
				editor.putString("userName",
						et_accountName.getText().toString()).commit();
				// 勾选自动登录的相关设置
				...
				if (cb_savePsw.isChecked())
				{
					...
					try
					{
						editor.putString(
								"password",
								AESUtil.encrypt(et_accountName.getText()
			...
			case LOGIN_AUTH_ERROR:
			{
				...
				Toast.makeText(LoginActivity.this, R.string.login_auth_error,
			...
			case LOGIN_NO_CONNECTION:
			{
				...
				Toast.makeText(LoginActivity.this,
						R.string.login_no_connection, Toast.LENGTH_SHORT)
						.show();
				et_psw.setText("");
				btn_login.setText(R.string.login);
				btn_login.setEnabled(true);
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		// 为注册按钮设置onclickListener
		btn_regNow.setOnClickListener(new Button.OnClickListener()
		{
			@Override
			public void onClick(View arg0)
			{
				Uri uri = Uri
						.parse("http://10.109.34.24/saas/index.php?app=user&ac=register");
		...
		btn_login.setOnClickListener(new Button.OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				final String cachePath = Environment
						.getExternalStorageDirectory() + "/Webdisk/cache/";// TODO
																			// 此路径需要修改
				...
				// try
				// {
				// editor.putString("password", AESUtil.encrypt(userName,
				// password)).commit();
				// } catch (Exception e)
				// {
				// e.printStackTrace();
				...
				// {
				// fileUrl =
				// SVNURL.parseURIEncoded("http://10.109.34.24/wangpan/" +
				// userName + "/.config.db");
				...
				// app.initAuthManager();
				//
				// String login_info = app.doExport(SVNRevision.HEAD,
				// cacheFolder, fileUrl);
				//
	...
	private void login()
	{
		if (et_accountName.getText().toString().equals("")
				|| et_psw.getText().toString().equals(""))
		{
			Toast.makeText(LoginActivity.this, R.string.pls_input_loginInfo,
					Toast.LENGTH_SHORT).show();
		} else
		{
			final String userName = et_accountName.getText().toString().replace("@", ".");
			Log.i(TAG, "userName after relplaced=" + userName);
			final String password = et_psw.getText().toString();
			final String cachePath = Environment.getExternalStorageDirectory()
					+ "/Webdisk/cache/";// TODO 此路径需要修改
			Log.i(TAG, "cachePath=" + cachePath);
			final File cacheFolder = new File(cachePath);
			...
			new Thread(new Runnable()
			{
				...
				public void run()
				{
					...
					if ("svn: Authentication required for '<http://10.109.34.24:80> hello svn'"
							.equals(login_info))// 登录失败
					{
						Message message = new Message();
						message.what = LOGIN_AUTH_ERROR;
						mHandler.sendMessage(message);
